...
class SiftDetector {
    ...
    public static class Parameters {
        ...
        /**
         * Max. principal curvature ratio used to eliminate line-like structures
         * (3..10)
         */
        public double rho_Max = 10.0;
        /**
         * Number of orientation bins in the feature descriptor (angular
         * resolution)
         */
        ...
        /**
         * Min. value in orientation histogram for dominant orientations (rel.
         * to max. entry)
         */
        ...
        /**
         * Scale factor for converting normalized features to byte values in
         * [0,255]
         */
    ...
    /**
     * Only used for debugging: produces key points with orientation histograms
    ...
    public List<SiftFeature> getSiftFeatures() {
        ...
        List<SiftFeature> siftDescriptors = new ArrayList<SiftFeature>();
        for (KeyPoint c : keyPoints) {
            for (double phi_d : getDominantOrientations(c)) {
                SiftFeature sd = makeSiftDescriptor(c, phi_d);
                if (sd != null) {
...
//				final float[][] invHess = Matrix.inverse3x3(hess);	// deprecated
                ...
                if (Math.abs(xx) < 0.5f && Math.abs(yy) < 0.5f) {	// stay at this lattice point
                    ...
                    if (Math.abs(Dpeak) > tPeak && detHxy > 0) { // check peak magnitude
                        ...
                        if (a <= aMax) { 					// check curvature ratio (edgeness)
                            if (params.DEBUG) {
                                IJ.log(k.toString() + String.format(": added after %d tries, alpha = %f", n, a));
                            }
    ...
    boolean isLocalMin(final float[][][] neighborhood, float tExtrm) {
        ...
        if (c >= 0) {
            return false;	// local minimum must have a negative value
    ...
    boolean isLocalMax(final float[][][] neighborhood, float tExtrm) {
        final float c = neighborhood[1][1][1] - tExtrm; 	// center value - threshold
        if (c <= 0) {
            return false;	// local maximum must have a positive value
        }		// check 8 neighbors in scale plane q
        if (c <= neighborhood[1][0][0]) {
            return false;
        }
